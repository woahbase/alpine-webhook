...
VERSION   ?= $(call get_gh_version,adnanh/webhook)
...
# runtime flags
MOUNTFLAGS := # -v $(CURDIR)/config:/etc/webhook
PORTFLAGS  := -p 9000:9000# --net=host # so other local containers can find it without explicit linking, needs firewall cleared
PUID       := $(shell id -u)
